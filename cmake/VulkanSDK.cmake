...
#
# Aperture requires a 1.4+ SDK (see APERTURE_MIN_VULKAN_SDK_VERSION). Slang
# has been bundled since 1.3.296; we reuse it from a 1.4+ SDK when
# APERTURE_SLANG_USE_VULKAN_SDK is ON.
#
...
#   APERTURE_VULKAN_SDK_VERSION      - e.g. "1.4.354"
...
function(_aperture_find_vulkan_sdk)
  ...
  set(_header "${_include_dir}/vulkan/vulkan_core.h")

  file(STRINGS "${_header}" _patch_line REGEX "^#define VK_HEADER_VERSION ")
  if(NOT _patch_line)
    message(STATUS "Vulkan SDK: could not parse VK_HEADER_VERSION from \"${_header}\"")
    return()
  endif()
  ...
  elseif(_complete_line MATCHES "VK_MAKE_VERSION\\(([0-9]+),[\t ]*([0-9]+)")
    set(_major "${CMAKE_MATCH_1}")
    set(_minor "${CMAKE_MATCH_2}")
  elseif(EXISTS "${_include_dir}/vulkan/vulkan.h")
    file(STRINGS "${_include_dir}/vulkan/vulkan.h" _api14 REGEX "^#define VK_API_VERSION_1_4")
    if(_api14)
      set(_major "1")
      set(_minor "4")
    endif()
  ...
  if(NOT _major)
    message(STATUS "Vulkan SDK: could not parse major.minor from \"${_header}\"")
    return()
  endif()
